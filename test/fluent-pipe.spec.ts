...
import { o, fluent, identity, interval, od, getGroupingDistinct, fluentPipe, fluentOp } from '../src';
    ...
    describe('synchronous', () => {
      context('withIndex', () => {
        it('should return Indexed instances from informed array', () => {
          expect(fluent(fluentPipe(['a', 'b', 'c'], fluentOp.withIndex())).toArray()).to.be.eql([
            { idx: 0, value: 'a' },
            { idx: 1, value: 'b' },
            { idx: 2, value: 'c' },
          ]);
      ...
      context('takeWhile', () => {
        it('works with initially not true statement', () =>
          expect(
            fluent(fluentPipe(subject, fluentOp.takeWhile((p: any) => p.emails.length > 0)))
              .toArray(),
          ).to.be.empty);
